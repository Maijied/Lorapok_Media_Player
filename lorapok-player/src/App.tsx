...
function App() {
  ...
  useEffect(() => {
    ...
    // Register widget provider for Android
    try {
      WidgetBridgePlugin.setRegisteredWidgets({
        widgets: ['com.lorapok.player.MediaWidgetProvider']
      });
  ...
  useEffect(() => {
    const syncMobileState = async () => {
      try {
        const title = filePath ? filePath.split(/[/\\]/).pop() : 'Lorapok Player';
        await MediaSession.setMetadata({
          title: title || 'Lorapok Player',
        ...
        await WidgetBridgePlugin.setItem({
          key: 'media_title',
          value: title || 'Lorapok Player',
          group: 'group.lorapok.widget'
        });
        await WidgetBridgePlugin.setItem({
          key: 'media_playing',
          value: isPlaying ? 'true' : 'false',
          group: 'group.lorapok.widget'
        });
  ...
  // Global Shortcut: ESC to Stop & Home
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // Escape to Stop & Home
      if (e.key === 'Escape') {
        if (filePath || isPlaying) {
          console.log('🛑 Stopping Playback (User Request)')
          setFilePath(null)
          setIsPlaying(false)
          setPlaylist([])
          setCodecError(null)
          // Ensure video element stops if it handles its own state
          if (videoRef.current) {
  ...
  }, [filePath, isPlaying, showStreamInput])
  ...
  // Smart Resume is handled via handleTimeUpdate (save) and the load-position effect below
  ...
  // Smart Resume: Load saved position on file change
  useEffect(() => {
    if (filePath && !incognitoMode) {
      ...
      if (savedTime && videoRef.current) {
        ...
        if (time > 5) {
          ...
          setCurrentTime(time)
          console.log(`[SmartResume] Restored ${baseIp} to ${time}s`)
        }
      }
    }
  }, [filePath])
  ...
  useEffect(() => {
    ...
    if (isStream) {
      if (isM3U8) {
            ...
            if (data.fatal) {
              switch (data.type) {
                ...
                case Hls.ErrorTypes.NETWORK_ERROR:
                  console.warn('[HLS] Network error, retrying in 2s...')
                  setTimeout(() => {
                ...
                default:
                  ...
                  hls.destroy()
                  setCodecError(`Fatal Stream Error: ${data.details}`)
                  setIsBuffering(false)
                  break
              }
            }
        ...
        player.initialize(videoRef.current, filePath, false)
        ...
        player.on(MediaPlayer.events.BUFFER_EMPTY, () => {
          setIsBuffering(true)
        })
        ...
        // Direct stream (MP4, WebM etc) - video element handles it natively
        setIsBuffering(false)
      }
    } else {
  ...
  // Buffer Stall Detection & Auto-Recovery
  useEffect(() => {
    if (!filePath || !videoRef.current) return
    ...
    const checkStall = () => {
      ...
      if (video.currentTime === lastTime && lastTime >= 0) {
        stallCount++
        console.warn(`[StallDetector] Stall detected (count: ${stallCount})`)

        if (stallCount >= 2) {
          // Try recovery
          if (hlsRef.current) {
            console.log('[StallDetector] Recovering HLS media error...')
            hlsRef.current.recoverMediaError()
          } else if (dashRef.current) {
            // DASH: try seeking forward slightly
            video.currentTime = video.currentTime + 0.1
          } else {
            // Native: nudge playback
            video.currentTime = video.currentTime + 0.1
          }
          stallCount = 0
        }
    ...
    stallTimer = setInterval(checkStall, 3000)
    return () => { if (stallTimer) clearInterval(stallTimer) }
  }, [filePath])

  // Buffering Overlay Safety Timeout
  useEffect(() => {
    ...
    if (videoElement.error) {
      switch (videoElement.error.code) {
        ...
        case videoElement.error.MEDIA_ERR_SRC_NOT_SUPPORTED:
          // Auto-retry with transcoding for BOTH local files and remote streams
          // If native playback fails, we wrap in media:// (or append transcode=true) to force FFmpeg
          if (filePath && (window as any).ipcRenderer) {
            // Avoid infinite loops: check if already transcoding
            const isAlreadyTranscoding = filePath.includes('transcode=true') || (filePath.startsWith('media://') && !filePath.match(/^https?:\/\//));

            if (!isAlreadyTranscoding) {
              console.log("⚠️ Native playback failed. Switching to Neural Decode...");
              let newPath = filePath;

              // If it's a remote URL being played natively, wrap it
              if (filePath.match(/^https?:\/\//)) {
                newPath = `media://${filePath}`;
              } else {
                // Local file: append transcode flag
                const sep = filePath.includes('?') ? '&' : '?';
                newPath = `${filePath}${sep}transcode=true`;
              }

              if (newPath !== filePath) {
                setFilePath(newPath);
                setCodecError("Handshaking with Neural Decoder...");
                return;
              }
            }
          }
                  ...
                  <div className="relative z-10 w-full flex flex-col gap-2">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <button onClick={() => setShowAudioLibrary(true)} className={`transition-colors ${showAudioLibrary ? 'text-neon-magenta' : 'text-white/30 hover:text-white/70'}`} title="Audio Library">
