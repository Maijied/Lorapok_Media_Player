...
/**
 * Get the correct MIME type for a media file based on extension.
 * Returns a sensible default for unknown types.
 */
...
if (!gotTheLock) {
  ...
  app.on('second-instance', (_event, commandLine, workingDirectory) => {
    if (win) {
      ...
      const protocolUrl = commandLine.find(arg => arg.startsWith('lorapok://'))
      if (protocolUrl) {
      ...
      if (possibleFile) {
        const absolutePath = path.isAbsolute(possibleFile) ? possibleFile : path.join(workingDirectory, possibleFile);
...
// Advanced Desktop Directory Scanner
ipcMain.handle('scan-directory', async (_event, directoryPath) => {
  logToFile(`Scanning directory for audio: ${directoryPath}`);
  const results: any[] = [];
  const audioExts = ['.mp3', '.flac', '.wav', '.aac', '.m4a', '.ogg', '.opus', '.wma'];
  ...
  async function walk(dir: string) {
    try {
      ...
      logToFile(`Scan error in ${dir}: ${e.message}`);
    }
  }

  // If no directory provided, default to the user's Music folder
  if (!directoryPath) {
    directoryPath = app.getPath('music');
  }
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      localServer = http.createServer((req, res) => {
        ...
        const contentType = getMediaMimeType(realPath)
    ...
    protocol.handle(p, async (request) => {
      ...
      try {
        ...
        if (nativeSupport) {
          ...
          const contentType = getMediaMimeType(decodedPath)
          if (!range) {
